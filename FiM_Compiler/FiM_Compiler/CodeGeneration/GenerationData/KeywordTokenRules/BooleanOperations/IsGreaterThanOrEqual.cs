...
{
    public class IsGreaterThanOrEqual : TokenRule
    {
        public IsGreaterThanOrEqual()
        {
            returnType = TokenType.GreaterThanOrEqual;
            rule = new TokenType[] {
        ...
        public override bool IsStackMatch(List<Token> stack)
        {
            if (DefaultStackCheck(stack, rule))
            {
                if (KeywordsDictionary.IsKeyword(KeywordType.BooleanComparatorNot, stack[stack.Count - 5].Value)
                    && KeywordsDictionary.IsKeyword(KeywordType.BooleanLessThan, stack[stack.Count - 3].Value))
                {
